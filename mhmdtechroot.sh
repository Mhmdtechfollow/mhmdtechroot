...
# Copyright ©2019 by mhmdtech. All rights reserved.
#
...
DESTINATION=${PREFIX}/share/mhmdtech
choice=""
...
addresolvconf ()
{
  ...
  [ \$(command -v getprop) ] && getprop | sed -n -e 's/^\[net\.dns.\]: \[\(.*\)\]/\1/p' | sed '/^\s*$/d' | sed 's/^/nameserver /' > \${PREFIX}/share/mhmdtech/etc/resolv.conf
  fi
}
addresolvconf
exec proot --link2symlink -0 -r \${PREFIX}/share/mhmdtech/ -b /dev/ -b /sys/ -b /proc/ -b /sdcard -b /storage -b \$HOME -w /home /usr/bin/env HOME=/root PREFIX=/usr SHELL=/bin/sh TERM="\$TERM" LANG=\$LANG PATH=/bin:/usr/bin:/sbin:/usr/sbin /bin/sh --login
EOM
...
finalwork() {
	[ ! -e ${DESTINATION}/mhmdtech.sh ] && curl --silent -LO https://raw.githubusercontent.com/mhmdtechfollow/mhmdtechroot/master/mhmdtech.sh
	if [ "${MOTD}" = "ON" ]; then
		bash ${DESTINATION}/mhmdtech.sh --add-motd
	else
		bash ${DESTINATION}/mhmdtech.sh
	fi
	rm ${DESTINATION}/mhmdtech.sh
}
...
cleanup() {
	...
	if [ -e ${PREFIX}/bin/mhmdtech ]; then
		rm ${PREFIX}/bin/mhmdtech
...
printline
printline
printf "\n${blue} [*] Email   :${yellow}    mojoker9999@gmail.com\n"
printf "$blue [*] GitHub :${yellow}    https://github.com/mhmdtechfollow\n"
printline
